...
PlayerStatus = (function() {
  ...
  // constants
  ACTUALLY_FALSE = 0; // because booleans in javascript are lies
  ACTUALLY_TRUE = 1;  // booleans = lies
  FIELD_SEPARATOR = "#"; // separates fields within player entries
  ENTRY_SEPARATOR = "|"; // separates player entries
  COUNTDOWN_INTERVAL = 3000; // delay between timer updates
  TIMER_LENGTH = 24; // the number of characters in the timer
  TIMER_QUANTUM = 2; // number of chars in the timer that change with each update
  CAO_PLAYER_URL = "http://crawl.akrasiac.org/scoring/players/"; // generates link to player page
  WEBTILES_SERVER_NAME = "CDO/Web"; // determines if a game is watchable
  WEBTILES_URL = "https://tiles.crawl.develz.org/#watch-"; // generates link to watch webtiles games
  
  // control variables
  is_offline = offline;
  game_data = []; // stores table data
  sort_category = "Player"; // default sorting column
  countdown_timer = TIMER_LENGTH; // initial length of countdown timer
  splash_screen = ACTUALLY_TRUE; // determines where to write "Loading games..."
  sort_reversed = ACTUALLY_FALSE; // whether the sorting column is sorted in reverse
  timer = null; // the timer object
  timer_is_on = ACTUALLY_FALSE; // determines whether the timer is active
